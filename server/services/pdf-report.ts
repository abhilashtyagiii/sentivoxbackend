...
export function generateDetailedPDFReport(data: DetailedReportData): Uint8Array {
  ...
  if (data.interview.jdAnalysis) {
    ...
    if (jdData.questionRelevance && jdData.questionRelevance.length > 0) {
      ...
      for (let i = 0; i < maxPairs; i++) {
        ...
        addBulletPoint(`Q${i + 1}: ${question.question?.substring(0, 60)}${question.question?.length > 60 ? '...' : ''} (Score: ${question.relevanceScore || 0}/100)`);
        
        if (answer) {
          addBulletPoint(`Response: ${answer.answer?.substring(0, 60)}${answer.answer?.length > 60 ? '...' : ''} (Alignment: ${answer.alignmentScore || 0}/100)`, 1);
        }
        yPosition += 2;
      }
      yPosition += 5;
  ...
  if (data.interview.flowAnalysis) {
    ...
    if (flowData.insights && flowData.insights.length > 0) {
      ...
      yPosition += 3;
      flowData.insights.slice(0, 3).forEach((insight: string) => {
        addBulletPoint(insight.substring(0, 120) + (insight.length > 120 ? '...' : ''));
      });
      yPosition += 5;
    }
...
function generateRecruiterRecommendations(data: DetailedReportData): string[] {
  ...
  const sentimentData = data.interview.sentimentAnalysis as any;
  const flowData = data.interview.flowAnalysis as any;
  const transcription = data.interview.transcription as any;
