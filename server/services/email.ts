...
class EmailService {
  ...
  constructor() {
    this.fromEmail = process.env.EMAIL_USER || 'shankeraishwarya03@gmail.com';
    this.initialize();
  }

  private initialize() {
    const emailUser = process.env.EMAIL_USER || 'shankeraishwarya03@gmail.com';
    const emailPassword = process.env.EMAIL_PASSWORD;

    if (!emailPassword) {
      console.warn('⚠️  EMAIL_PASSWORD not set. Email sending will be simulated.');
      return;
    }
...
╚════════════════════════════════════════════════════════════╝
    ...
    try {
      const info = await this.transporter.sendMail({
        from: `"Sentivox AI Platform" <${this.fromEmail}>`,
        to: config.to,
