...
export async function processEnhancedInterview(interviewId: string): Promise<void> {
  ...
  try {
    ...
    // Run these 3 API calls in PARALLEL
    ...
    // Add resume analysis if resume is available
    ...
    const [jdResult, baseFlowResult, resumeAnalysisResult] = parallelResults;
    ...
    steps[currentStepIndex].message = `✅ Parallel analysis complete: JD, Flow${resumeAnalysisResult ? ', Resume' : ''} (saved ~25s)`;
    ...
    if (resumeAnalysisResult) {
      ...
      updateData.candidateReport = resumeAnalysisResult.candidateReport;
      updateData.recruiterReport = resumeAnalysisResult.recruiterReport;
    }
  ...
  for (const segment of segments) {
    if (segment.speaker === "Recruiter" && segment.text.includes("?")) {
      ...
      const matchingQuestion = jdResult.questionRelevance?.find((q: any) => 
        q.question && segment.text.includes(q.question.substring(0, 30))
      );
      ...
      const matchingAnswer = jdResult.answerAlignment?.find((a: any) => 
        a.answer && segment.text.includes(a.answer.substring(0, 30))
      );
