...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/interviews/:id/candidate-report", async (req, res) => {
    try {
      ...
      if (!candidateData) {
        ...
        const strengthAreas = Array.isArray(insights) ? insights.filter((i: string) => 
          i.toLowerCase().includes('strength') || 
          i.toLowerCase().includes('excellent') || 
          i.toLowerCase().includes('good')
        ).slice(0, 3) : [];
        
        const improvementAreas = Array.isArray(insights) ? insights.filter((i: string) => 
          i.toLowerCase().includes('improve') || 
          i.toLowerCase().includes('consider') || 
          i.toLowerCase().includes('develop')
        ).slice(0, 3) : [];
  ...
  app.patch("/api/interviews/:id/outcome", async (req, res) => {
    try {
      ...
      if (!outcome || !['pending', 'next_stage', 'rejected'].includes(outcome)) {
        return res.status(400).json({ message: "Invalid outcome. Must be 'pending', 'next_stage', or 'rejected'" });
      }
      ...
      res.json({ interview: updatedInterview, message: "Candidate outcome updated successfully" });
    } catch (error) {
      console.error('Update outcome error:', error);
      res.status(500).json({ message: error instanceof Error ? error.message : 'Failed to update outcome' });
    }
  });
